...
def main():
    ...
    if image_file is not None:
        ...
        st.subheader('Select Operation Type:')
        operation_type = st.radio('', ['Basic Operations', 'Edge Detection Techniques'])

        if operation_type == 'Basic Operations':
            st.subheader('Basic Operations')
            basic_operation = st.radio('Select Operation', ['None', 'Invert', 'Grayscale', 'Resize', 'Rotate'])
            output = apply_basic_operations(img, basic_operation)
        elif operation_type == 'Edge Detection Techniques':
            st.subheader('Edge Detection Techniques')
            edge_method = st.radio('Select Edge Detection Method', ['None', 'Canny', 'LOG', 'DOG'])
